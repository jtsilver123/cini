-- Letterboxd diaries log every rewatch as its own row; the importer now
-- sends them all, so each becomes its own diary entry (still skipping
-- exact duplicates on re-import). watched_on stays accepted for
