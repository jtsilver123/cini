-- Move direct-rec resolution from the blanket AFTER INSERT trigger (0109) into
-- rank_insert, so it respects stealth. A stealth rank is "hidden from your
-- friends' feeds" — it must NOT tell the recommender you watched their pick.
-- The rec is still cleared from your own Friend Recs either way (that's your
-- list, not friend-facing). rank_insert is the sole writer of `rankings`, so
-- every rank path (app, import, Ask Cini) still resolves recs.

drop trigger if exists rankings_resolve_direct_recs on rankings;
drop function if exists public.resolve_direct_recs_on_rank();
