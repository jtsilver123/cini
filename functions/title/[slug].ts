// SSR canonical title page: /title/<slug> — the single keyword-rich URL per
// title, server-rendered with Cini's LIVE community score, who ranked it, and
// notes. The old /m/?id= and /reviews/<slug>/ surfaces 301 here (see
// functions/m/index.ts and _redirects), so each title has one canonical page.
import { rpc, esc, tmdb, safeHttps, scoreColor, shell, notFoundPage, OG_FALLBACK } from "../_lib";

interface Title {
  tmdb_id: number; slug: string; media_kind: string; title: string; release_year: number | null;
...
export const onRequestGet: PagesFunction = async (context) => {
  ...
  const canonical = `https://trycini.com/title/${esc(t.slug || slug)}`;
  ...
  const body = `
    ...
    <div class="head" style="margin-top:${backdrop ? "-40px" : "6px"};position:relative;">
      ...
      <div><h1 class="title" style="margin:0;">${esc(t.title)}</h1>
    ...
    <div class="foot"><a href="cini://movie?id=${t.tmdb_id}">Rank it in Cini</a>
